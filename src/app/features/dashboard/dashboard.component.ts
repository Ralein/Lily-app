...
@Component({
  ...
  standalone: true,
  imports: [CurrencyDisplayPipe, RelativeDatePipe, NumberAnimateDirective, QuickAddComponent, BaseChartDirective],
  template: `
    <div class="dashboard">
      ...
      <div class="lily-card lily-card--hero hero-card animate-fade-in-up">
        ...
        <div class="hero-card__stats">
          <div class="hero-card__stat hero-card__stat--income">
            <span class="hero-card__stat-dot" style="background: var(--color-emerald)"></span>
          ...
          <div class="hero-card__stat hero-card__stat--expense">
            <span class="hero-card__stat-dot" style="background: var(--color-rose)"></span>
      ...
      <div class="grid grid--3">
        ...
        <div class="lily-card lily-card--gradient animate-fade-in-up stagger-3">
          ...
          @if (currentInsight(); as insight) {
            <p class="tip-text">{{ insight.emoji }} {{ insight.text }}</p>
          } @else {
            ...
            @for (item of budgetPulseData(); track item.categoryId) {
              <div class="budget-pulse__item">
                <div class="budget-pulse__info">
                  <span class="budget-pulse__cat">{{ item.icon }} {{ item.name }}</span>
        ...
        @if (store.recentTransactions().length > 0) {
          <div class="txn-list">
            @for (txn of store.recentTransactions(); track txn.id) {
              <div class="txn-row">
                <div class="txn-row__icon" [style.background]="getCategoryColor(txn.categoryId)">
                ...
                <button class="txn-row__delete" (click)="deleteTransaction(txn.id)" aria-label="Delete transaction">✕</button>
              </div>
            }
          </div>
        } @else {
  ...
  styles: [`
    ...
    .hero-card {
      ...
      &__stat-dot { width: 8px; height: 8px; border-radius: var(--radius-full); }
    ...
    .budget-pulse__cat { font-size: var(--fs-sm); font-weight: var(--fw-medium); }
    ...
    .txn-row__icon {
      ...
      font-size: 1.25rem; flex-shrink: 0; opacity: 0.2;
    }
    ...
    .txn-row__delete {
      opacity: 0; padding: var(--space-1); color: var(--color-text-tertiary); font-size: var(--fs-xs);
...
export class DashboardComponent {
  ...
  getCategoryIcon(id: string): string { return this.store.categoryMap().get(id)?.icon || '📦'; }
